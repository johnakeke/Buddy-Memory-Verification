...
\title{A Verified Buddy Memory Allocation Model}
...
\begin{abstract}
Buddy allocation algorithms are widely adopted by various memory management systems for managing memory layouts. Rigorous mathematical proofs provide strong assurance to improve confidence on the reliability of a memory management. In this paper, we offer a verified buddy memory allocation model which preserves functional correctness and security property by formal methods. Firstly, we use interactive theorem prover Isabell/HOL to construct a specification for the buddy allocation algorithm consisting of operations to allocate and dispose memory blocks. Then we verify that the specification preserves key invariants over the memory to guarantee functional correctness of the algorithm. Finally, we verify that the specification also preserves integrity of the memory. Therefore, they do not affect other memory blocks previously allocated.
\end{abstract}
